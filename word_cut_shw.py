...
def test(img_path, points, save_path):
	...
	boxes, probs = predict(im_list)
	print("Boxes: {}".format(boxes))
	debug_img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
	# text = open(os.path.join(save_path, "probability_{}.txt".format(nth)), 'w')
	for i, value in enumerate(boxes):
		if (value==1):
			cv2.rectangle(debug_img, (points[i][0],0), (points[i][1], h), thickness=2, color=(255,0,0))
		# text.write("box: {} Japanese prob: {} Latin prob: {}\n".format(i, probs[i][0], probs[i][1]))
		print(probs[i])
	# text.close()
	# plt.imshow(debug_img)
	# plt.show()

	cv2.imwrite(os.path.join(save_path, "{}_debug.png".format(name)), debug_img)

def get_segment(img_path, img, points, save_path, nth):
	h, w = np.shape(img)[:2]
	direction, name = os.path.split(img_path)
	...
	[cv2.line(sep_img, (p,0), (p,h), (255,0,0), 2, -1) for p in points]
	cv2.imwrite(os.path.join(im_dir, "linecut_{}.png".format(nth)), sep_img)
	# cv2.imwrite(os.path.join(im_dir, "line_{}.png".format(nth)), img)

	cut_points = [[p1, p2] for p1, p2 in zip(points[:-1], points[1:])]
...
def main(img_path, save_path, nth=0):
	...
	# get_segment(img_path, img, sep_points, save_path, nth)
	test(img_path, cut_points, save_path)

	# sep_img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
	# [cv2.line(sep_img, (p,0), (p,h), (255,0,0), 2, -1) for p in points]
	# f, axarr = plt.subplots(2,1)
	# axarr[0].imshow(binary)
	# axarr[1].imshow(sep_img)
	# plt.show()
...
if __name__ == "__main__":
    ...
    print("**************", name)
    save_folder = os.path.join(os.path.normpath(args.path + os.sep + os.pardir), "{}_debug".format(name))
    # save_folder = os.path.join(os.path.normpath(args.path + os.sep + os.pardir), "{}_wordcut".format(name))
